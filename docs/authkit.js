...
(() => {
  ...
  (function() {
    ...
    const config = {
      clientId,
      redirectUri,
      apiBaseUrl: dataset.apiHostname ? `https://${dataset.apiHostname}` : "https://api.workos.com",
      devMode: dataset.devMode !== void 0 ? dataset.devMode === "true" : location.hostname === "localhost" || location.hostname === "127.0.0.1",
    ...
    const isCallbackUrl = (function() {
      try {
        ...
        return (currentPath === redirectPath || currentPath === redirectPath + "/") && new URLSearchParams(window.location.search).has("code");
      } catch {
        return false;
      }
    })();
